...
由于物体的高度，反射率的不同，相机拍摄的变形条纹图像可以用下式表示：
$$
\begin{align*}
...
\begin{align*}
    ...
    &= A(x,y) + B(x,y)\left[\cos\varphi\cos\frac{2k\pi}{N}
    +\sin\varphi\sin\frac{2k\pi}{N}\right] \\
...
对于求解方程，只需要三个方程就行（因为只有三个未知数）。但是由于噪声，使用的条纹图数量越多，就越能够提高求解精度。当方程数量大于变量数量，方程组变成了超定方程，求解超定方程，我们可以使用最小二乘法。
...
\\\\
    A^T &=\begin{bmatrix}
    1 & \dots & 1 & \dots & 1\\\\
    \cos\frac{2\cdot0\cdot\pi}{N} & \dots & \cos\frac{2\cdot k\cdot\pi}{N} & \dots & \cos\frac{2\cdot (N-1)\cdot\pi}{N}\\\\
...
\end{bmatrix}\\\\
    ...
    \frac{2}{N}\cos\frac{2\cdot0\cdot\pi}{N} & \dots & \frac{2}{N}\cos\frac{2\cdot k\cdot\pi}{N} & \dots & \frac{2}{N}\cos\frac{2\cdot (N-1)\cdot\pi}{N}\\\\
